        ...
        dbc.Row(
            dbc.Col(
                [
                    # description
                    html.Div(
                        children="""Select a pokemon to see its information.""",
                        style={"textAlign": "center"},
                    ),
                    # dropdown
                    dbc.Row(
                        [
                            ...
                            dbc.Col(dbc.Button("Next Evolution", id="next-button")),
                        ],
            ...
            [
                dbc.Col(
                    dbc.Card(
                        [
                            dbc.CardImg(
                                src="assets/ditto.png", id="card-img", top=True
                            ),
                            dbc.CardBody(
                            ...
                            dbc.CardBody(
                                [
                                    ...
                                    dash_table.DataTable(
                                        ...
                                        columns=[
                                            {"name": col, "id": col}
                                            for col in pokemon.columns[8:10]
                                        ],
                                        style_table={
                                            "width": "100%",
                                            "margin": "auto",
                                    ...
                                    dash_table.DataTable(
                                        ...
                                        columns=[
                                            {"name": col, "id": col}
                                            for col in pokemon.columns[10:16]
                                        ],
                                        style_table={
                                            "width": "100%",
                                            "margin": "auto",
...
def update_components(selected_value):
    if selected_value is not None:
        return [
            pokemon[pokemon["Pokemon"] == selected_value].to_dict("records"),
            pokemon[pokemon["Pokemon"] == selected_value][["Type 1", "Type 2"]].to_dict(
                "records"
            ),
            pokemon[pokemon["Pokemon"] == selected_value][
                [
                    "HP",
                    "Attack",
                    "Defense",
...
def evolve_pokemon(n_clicks, current_pokemon):
    if n_clicks > 0:
        evolves_to = pokemon.loc[
            pokemon["Pokemon"] == current_pokemon, "Evolves To"
        ].values[0]
        return evolves_to
    else:
